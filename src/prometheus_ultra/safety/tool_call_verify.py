"""ToolCallVerifier — 工具调用参数完整性验证。

检查计划调用的工具参数与实际执行的参数是否一致。
基于字符串相似度和敏感模式检测参数替换。

注意: 本文件曾引用 arXiv 2605.26154 (MemMorph)，该论文描述
通过记忆中毒劫持工具选择的攻击方法。当前实现仅做参数差异
比较，不实现 MemMorph 的记忆中毒攻击管线。

功能:
- 比较计划参数与实际参数
- 检测参数值的差异（新增/删除/修改）
- 对敏感路径/URL/DB参数的差异标记为高严重度
"""
