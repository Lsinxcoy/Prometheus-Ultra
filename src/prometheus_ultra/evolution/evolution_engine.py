...
class CrossoverEngine:
    ...
    def _multi_point(g1: List[Gene], g2: List[Gene],
                     p1: Chromosome, p2: Chromosome) -> Tuple[Chromosome, Chromosome]:
        n_points = random.randint(2, min(4, len(g1)))
        points = sorted(random.sample(range(1, len(g1)), n_points))
        child1_genes = []
        child2_genes = []
        use_g1 = True
        ...
        for _ in range(len(population)):
            ...
            for c, p in zip(population, probabilities):
                ...
                if r <= cumulative:
                    selected.append(c)
                    break
            if not selected or len(selected) < len(population):
...
class Terminator:
    ...
    def __init__(self, max_generations: int = 100, stagnation_limit: int = 20,
                 fitness_threshold: float = 0.99, min_generations: int = 5):
