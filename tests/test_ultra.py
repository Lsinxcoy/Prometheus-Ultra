...
from prometheus_ultra import Omega, ZConfig, generate_uuidv7
...
class TestLearning:
    def test_knowledge_scanner(self, omega):
        from prometheus_ultra.learning.scanner import ScanSource
        results = omega.knowledge_scanner.scan(ScanSource.WEB, "test query", max_results=2)
        assert len(results) >= 1
